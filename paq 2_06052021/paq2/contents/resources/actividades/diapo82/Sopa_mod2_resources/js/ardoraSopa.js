...
context.fill();context.lineWidth = 2;context.strokeStyle = "#09BBE1";/*Color del borde del sleccionar palabra*/
...
var title="P A L A B R A S:";
context.font = "12px "+fActi;/*Tamño de las palabras a encontrar*/
var widthWords=context.measureText(title).width+20;
var lineWord=sizeCell;
context.fillStyle = "#000";/*Color de las palabras a encontrar*/
context.textAlign = "left";
lineWord=lineWord+sizeCell;
for (j = 0; j < words.length; j++) {
	if (context.measureText(words[j]).width>widthWords){
		widthWords=context.measureText(words[j]).width+20
	}
context.save();
context.beginPath();
context.lineWidth=0.3;/*Opacidad de cuadro al frente de palabra*/
context.fillStyle ="#FFFDFD";/*color de cuadro al frente de palabra*/
context.rect(numCols*sizeCell+30,lineWord-sizeCell+10,sizeCell-5,sizeCell-8); /*estilo del cuadro al frente de palabra*/
context.fill();context.stroke();context.restore();
context.fillText(SopaWords(words[j]).substring(0,c[j]),numCols*sizeCell+30+sizeCell,lineWord); /* mover las palabras mas cerca de la margen izquierda*/
lineWord=lineWord+sizeCell;}context.save();
context.beginPath();
context.lineWidth=0.3; /*Boder del titulo*/
context.strokeStyle = colorText;context.fillStyle = colorButton;
roundedRect(context, numCols*sizeCell+10,0,widthWords,sizeCell, 4, "#FFF");/*Color de Boton y recuadro del titulo */
context.shadowColor="#FFF";/*sombra del title */
context.shadowBlur = 20;
context.shadowOffsetX = 3;
context.shadowOffsetY = 3;
context.fill();context.stroke();
context.fillStyle = colorText;context.font = "bold 12px "+fActi;/*Tamaño de la letra de title*/
context.fillText(title,numCols*sizeCell+20,sizeCell-5);context.restore();
}
