...
if (typeMessage=="Ok"){
	// window.parent.ok();
	var textMessage=messageOk; var urlMessage=urlOk; var goURLTarget= goURLOk; var borderColor=borderOk;
...
var actArea=document.getElementById("ardoraActCanvas");
document.getElementById("ardoraActCanvas").style.zIndex=5;
document.getElementById("ardoraActCanvas").style["visibility"]="visible";
document.getElementById("ardoraTab").style["visibility"]="hidden";
$("#ardoraActCanvas").css("cursor", "pointer");
if (tiTime || tiAttempts || tiScore || tiSuccesses ){
	clearInterval(timeInterval);}
	var canvas = document.getElementById("ardoraActCanvas");
var contexto = canvas.getContext("2d");
canvas.width = canvas.width;contexto.globalAlpha = 0.98;
...
contexto.strokeStyle = colorText;roundedRect(contexto,xAnim,y-20,wAnim,30,5,colorBack);
xAnim-=1;wAnim+=2;if (wAnim>metricsW+30){
...
$("#ardoraActCanvas").unbind("mousedown");
document.getElementById("ardoraActCanvas").style.zIndex=0;
document.getElementById("ardoraActCanvas").style["visibility"]="hidden";
document.getElementById("ardoraTab").style["visibility"]="visible";
paintBack();}});}
function cssColors(){
$("body").css("background-color",colorBack);/*Color de todo el  fondo*/
$("#ardoraMain").css("color",colorText);
if ($(".txtCell_input").length !=0) { 
	$(".txtCell_input").css("background-color",colorBack); 
	$(".txtCell_input").css("color",colorText);
}
if ($(".txtCell_txt").length !=0) {
 $(".txtCell_txt").css("background-color",colorBack)
}
$("#ardoraMain").css("font-family",fEnun).css("background-color","#fff"); /*parte superior de la pagina*/
$(".txtCell").css("font-family",fActi);
}
