...
for (var i = 0; i < 12; i++) {context.rotate(Math.PI * 2 / 12);context.beginPath();if (i==iL){context.lineWidth = 8;
	context.strokeStyle = "rgba(9, 187, 225," + i / 12 + ")";}
else{context.lineWidth = 3;
	context.strokeStyle = "rgba(50,50,50," + i / 12 + ")";
}
...
function selRectD2(ctx, sx, sy, ex, ey,color) {
	var r = sizeCell / 2;sx = sx + sizeCell;ey = ey + sizeCell;sy = sy + 2;ey = ey + 2;
	ctx.beginPath();
ctx.fillStyle = "rgba("+hexToRgb(color).r+","+hexToRgb(color).g+","+hexToRgb(color).b+",0.3)";
ctx.moveTo(sx - r, sy);
...
/**
 * [paintCheckOk diseño de chulito]
 * @param   {[type]}  ctx  [objeto de chulito]
 */
function paintCheckOk(ctx,x,y,w){
	ctx.save();ctx.beginPath();
	ctx.moveTo(x,y-w/2);
ctx.fillStyle = "rgb(9, 187, 225)";/*Color del chulito*/
ctx.quadraticCurveTo(x+w/3,y-w/3, x+w/2,y);
ctx.quadraticCurveTo(x+7*w/10,y-w/2,x+w,y-w);
ctx.quadraticCurveTo(x+7*w/10,y-7*w/10,x+w/2,y-w/3);
ctx.quadraticCurveTo(x+w/3,y-w/2,x,y-w/2);
ctx.closePath();
ctx.stroke();
ctx.fill();
ctx.restore();
